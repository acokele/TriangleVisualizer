...
///of the accordion's parent-control height and the of the sum of all heights of the:
///checkbox headers, margins, paddings, and preferred size heights of controls themselves.
...
public class Accordion : UserControl, IMessageFilter {
	...
	public Accordion() {
		...
		this.ShowToolMenuOnHoverWhenClosed = false;
		this.ShowToolMenuOnRightClick = true;
		this.GrabWidth = 6;
		this.GrabCursor = Cursors.SizeNS;
		this.GrabRequiresPositiveFillWeight = true;
		Controls.Add(host);
	...
	public bool PreFilterMessage(ref Message m) {
		...
		if (m.Msg == WM_MOUSEMOVE) {
			...
			if (isDragging) {
				...
				int newdh = Math.Max(0, originalDH + (pt.Y - grabPoint.Y));
			...
			Control2 c = FindControl2(pt);
			bool hit = bounds.Contains(pt);
	...
	private Control2 FindControl2(Point pt) {
		...
		foreach (Control2 c2 in host.Control2s) {
			...
			int y = b2.Y + b2.Height;
			int dy = y - pt.Y;
			// only grab when in the range and the control is open
			bool canGrab = !GrabRequiresPositiveFillWeight || GrabRequiresPositiveFillWeight && c2.fillWt > 0;
	...
	public override Size GetPreferredSize(Size proposedSize) {
		Size s = host.GetPreferredSize(proposedSize);
		return s;
	}
	...
	public CheckBox Add(Control c, String text, String toolTip = null, double fillWt = 0, bool? open = null, Padding? contentPadding = null, Padding? contentMargin = null, Color? contentBackColor = null, Padding? checkboxMargin = null, bool? collapsible = null) {
		ICheckBoxFactory cbf = CheckBoxFactory ?? GlobalCheckBoxFactory;
		bool check = open.HasValue ? open.Value : OpenOnAdd;
		CheckBox cb = cbf.CreateCheckBox(text, check, Get(checkboxMargin, CheckBoxMargin));
		// the unicode arrows cause a brief flicker of the scrollbar

        bool collapse = (collapsible.HasValue && collapsible.Value == true) || !collapsible.HasValue;
        string empty = "     ";

        if (collapse)
            cb.Text = (cb.Checked ? GetUpArrow() : GetDownArrow()) + cb.Text;
        else
            cb.Text =  empty + cb.Text;
		...
		Action<bool> layout = (ControlAdded) => {
			...
			if (this.HorizontalScroll.Visible && IsHandleCreated) {
			...
			if (this.VerticalScroll.Visible) {
				this.ScrollControlIntoView(c2);
				this.ScrollControlIntoView(cb);
			}
		};

		cb.CheckedChanged += delegate {
            String a = collapse ? (cb.Checked ? GetUpArrow() : GetDownArrow()) : empty;
			host.isAdjusting = true;
    ...
    public Control ControlForCheckBox(CheckBox header)
    {
        Accordion acc = this;
        for (int i = 0; i < acc.Count; ++i)
        {
            Control2 control = host.Controls[2 * i + 2] as Control2;
            if (control != null && control.cb == header)
                return control;
        }
	...
	private class FLP : FlowLayoutPanel {
		...
		public void UpdateDeltaHeights(bool fillLastOpened, bool fillModeGrowOnly, bool fillResetOnCollapse) {
			Size r = GetPreferredSize(false, true, false);
			Accordion a = (Accordion) Parent;
			Size cs = a.ClientSize;

			double totalWt = 0;
			int mh = 0; // minus height
			foreach (Control2 c2 in this.Control2s) {
				if (c2.cb.Checked) {
					if (c2.isLocked)
						mh += c2.dh;
					else
		...
		private class DummyControl : UserControl {
			...
			public override Size GetPreferredSize(Size proposedSize) {
				FLP host = (FLP) Parent;
				Accordion a = (Accordion) host.Parent;
				if (a == null)
					return Size.Empty;
				...
				int w = 0;
				for (int i = 1; i < host.Controls.Count; i++) {
					Size s = host.Controls[i].GetPreferredSize(proposedSize);
					if (s.Width > w)
						w = s.Width;
				}

				int w2 = a.ClientSize.Width - host.Padding.Horizontal;
				if (w2 > w && a.FillWidth)
					w = w2;
	...
	private class Control2 : UserControl {
		...
		internal Size GetPreferredSize(Size proposedSize, bool addDH, bool returnEmptyIfClosed) {
			...
			Size s2 = c.GetPreferredSize(proposedSize);
			Padding p = Padding;
	...
	private class ToolBox : ToolStripDropDown {
		...
		public ToolBox() {
			...
			Action<Object> fillAction = (src) => {
				Current.cb.Focus();
				FLP host = (FLP) Current.Parent;
				Accordion a = (Accordion) host.Parent;
				Size r = host.GetPreferredSize(true, true, false);
				Size cs = a.ClientSize;
				int eh = cs.Height - r.Height;
				...
				if (src == miFill) {
					if (eh > 0) {
						...
						int dhNew = Math.Max(Current.dh, (cs.Height - s.Height) - Current.cb.Height);
						Current.dh = dhNew;
			...
			Action<Object> packAction = (src) => {
				...
				if (src == miPack || src == miPackAll) {
					...
					Accordion acc = (Accordion) host.Parent;
					acc.Close(null);
				}
			};
    ...
    public class CheckBoxHeaderFactory : Accordion.ICheckBoxFactory
    {
        public virtual CheckBox CreateCheckBox(String text, bool check, Padding margin)
        {
            CheckBox cb = new CheckBox();
            ...
            cb.Anchor = AnchorStyles.Left | AnchorStyles.Right; // cb.Dock = DockStyle.Fill also works.
            cb.Margin = margin; // typically 0 so that no are gaps between the buttons
            //cb.BackColor = Color.FromArgb(40, 160, 240);
            cb.FlatStyle = FlatStyle.Standard;

            cb.Height = 40;

            cb.Font = new Font(cb.Font.Name, 9.25f,
                cb.Font.Style, cb.Font.Unit,
                cb.Font.GdiCharSet, cb.Font.GdiVerticalFont);

            CheckBox newCb = new CheckBox();
            newCb.Padding = new Padding { Top = 2, Right = 8 };
            newCb.BackColor = Color.Transparent;
            newCb.AutoSize = true;
            newCb.Dock = DockStyle.Right;

            cb.Controls.Add(newCb);

            Action<Control, bool> setCheckBoxState = null;

            setCheckBoxState = (control, state) =>
                {
                    if(control is CheckBox)
                        (control as CheckBox).Checked = state;

                    foreach(Control c in control.Controls)
                        setCheckBoxState(c, state);
                };
            ...
            newCb.CheckedChanged += (sender, e) =>
                {
                    ...
                    CheckBox header = me.Parent as CheckBox;
                    if (header == null) return;

                    Control flowPanel = header.Parent;
                    if (flowPanel == null) return;

                    Accordion accordion = flowPanel.Parent as Accordion;
                    if (accordion == null) return;

                    Control control2 = accordion.ControlForCheckBox(header);

                    if(me.CheckState != CheckState.Indeterminate)
                        setCheckBoxState(control2, me.Checked);
                };
...
public class AccordionTestForm : Form {
	...
	public AccordionTestForm() {
        acc.CheckBoxFactory = new CheckBoxHeaderFactory();

		this.Padding = new Padding(10);
		this.Controls.Add(splitPane);
		...
		//acc.Insets = new System.Windows.Forms.Padding(10);
		//acc.ContentPadding = new System.Windows.Forms.Padding(10);
		//acc.ContentMargin = new Padding(0);
		//acc.CheckBoxMargin = new System.Windows.Forms.Padding(0);
        acc.ShowToolMenu = false;
		...
		Control panel = new FlowLayoutPanel {  FlowDirection = FlowDirection.TopDown};
		var cb = new CheckBox { Text = "Check Box 1"};
        var cb_other = new CheckBox { Text = "Check Box 2"};
		//cb.BackColor = Color.Red;
		panel.Controls.Add(cb);
        panel.Controls.Add(cb_other);
        acc.Add(panel, "CheckBoxes");
		Color? blue = null;
        
		//acc.Add(panel, "Check Box", contentBackColor:blue, open:true);

		FlowLayoutPanel options = new FlowLayoutPanel() { Dock = DockStyle.Fill };
		Button btn = new Button {Text = "Add" };
		CheckBox cb1 = new CheckBox{Text = "Only One Open", AutoSize = true };
		CheckBox cb2 = new CheckBox { Text = "Fill Last", AutoSize = true };
		CheckBox cb3 = new CheckBox { Text = "Grow Only", AutoSize = true };
		CheckBox cb4 = new CheckBox { Text = "Fill Reset", AutoSize = true};
		CheckBox cb5 = new CheckBox { Text = "Expand", AutoSize = true };
		CheckBox cb6 = new CheckBox { Text = "Fill Wt", AutoSize = true };
        ...
        for(int i = 0; i < acc.Count; ++i)
        {
            Control control = acc.Content(i);
            foreach(Control child in control.Controls)
            {
                int j = i;
                if (child is CheckBox)
                    (child as CheckBox).CheckedChanged += (se, e) =>
                        {
                            Tick(j);
                        };
            }
